...
COMMENT ON COLUMN "oh_boot"."ur_trajectory"."accuracy" IS '位置的精确度，反应与真实位置之间的接近程度可以理解成10即与真实位置相差10m，越小越精确';
COMMENT ON COLUMN "oh_boot"."ur_trajectory"."altitude" IS '高度，单位 m';
COMMENT ON COLUMN "oh_boot"."ur_trajectory"."horizontal_accuracy" IS '水平精度，单位 m';
COMMENT ON COLUMN "oh_boot"."ur_trajectory"."longitude" IS '经度，范围为 -180~180，负数表示西经。使用 gcj02 国测局坐标系';
...
-- ----------------------------
-- Primary Key structure for table ur_trajectory
-- ----------------------------
ALTER TABLE "oh_boot"."ur_trajectory" ADD CONSTRAINT "ur_trajectory_pkey" PRIMARY KEY ("id");
